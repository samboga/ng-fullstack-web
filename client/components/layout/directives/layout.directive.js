...
angular.module('realchatApp.layout').directive('smartLayout', function($rootScope, $state, $timeout, $interval, $q, SmartCss, appConfig, Auth) {
  ...
  (function applyConfigSkin() {
    if (appConfig.smartSkin) {
      $body.removeClass(_.pluck(appConfig.skins, 'name').join(' '));
      $body.addClass(appConfig.smartSkin);
    }
  })();
  ...
  return {
    ...
    compile: function(tElement, tAttributes) {
      ...
      $rootScope.$on('$stateChangeStart', function(event, toState, toParams, fromState, fromParams) {
        ...
        if (toState.authenticate && !Auth.getCurrentUser().isLoggedIn) {
